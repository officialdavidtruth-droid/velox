...
export default function WebsiteAnalytics({ workspaceId }: WebsiteAnalyticsProps) {
  ...
  const connectGA4 = async () => {
    ...
    if (!google?.access_token) {
      setSyncError('No Google account connected. Go to Connect Accounts and connect Google/YouTube first — Velox Space needs that OAuth token to read your GA4 property.');
      return;
    }
    setConnecting(true);
        ...
        <div className="space-y-5">
              ...
              <div className="mt-4 space-y-3">
                <div className="rounded-xl p-4" style={{ background:'var(--surface)', border:'1px solid var(--border)' }}>
                  ...
                  <p className="text-[10px] mt-2" style={{ color:'var(--muted)' }}>
                    ⚠️ You must first connect Google (YouTube) from Connect Accounts so Velox Space has a valid Google OAuth token.
                  </p>
                </div>
              </div>
            ) : (
          ...
          {ga4Data.length === 0 && ga4Connected && (
            <div className="glow-card rounded-2xl p-12 text-center">
              ...
              <p className="text-xs" style={{ color:'var(--muted)' }}>Velox Space will pull your last 30 days of GA4 data</p>
            </div>
          )}
