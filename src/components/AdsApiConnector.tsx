...
const AD_PLATFORMS = [
  {
    ...
    color: '#1877f2',
    fields: [
      { name: 'ad_account_id', label: 'Ad Account ID', placeholder: '102074366824146', help: "Meta Ads Manager → Account Overview → top left dropdown. Just the number — Velox Space adds the 'act_' prefix automatically." },
...
export default function AdsApiConnector({ workspaceId }: AdsApiConnectorProps) {
  ...
  const handleSave = async (platform: any) => {
    ...
    try {
      ...
      if (d.success) {
        if (d.verified) {
          setMsg({ type: 'success', text: 'Connected and verified! Real data will sync now.' });
          load();
          setTimeout(() => { setActiveForm(null); setMsg(null); }, 2000);
        } else {
          setMsg({ type: 'error', text: `Saved, but verification failed: ${d.verifyError || 'unknown reason'}. Double check the credentials and try again — it won't pull real data until verified.` });
          load();
        }
      } else {
