...
export default function CampaignTracker({ workspaceId }: CampaignTrackerProps) {
  ...
  const syncFromApi = async () => {
    ...
    try {
      ...
      const d = await r.json();
      if (d.platformResults) setPlatformResults(d.platformResults);
      if (d.imported > 0) {
        setApiSyncMsg({ text: `✓ Imported ${d.imported} campaign${d.imported!==1?'s':''} from your ad accounts.`, ok: true });
        load();
      } else if (d.success) {
        setApiSyncMsg({ text: 'Connected but no campaigns found. Check your ad accounts have active campaigns.', ok: false });
      } else {
        setApiSyncMsg({ text: d.error || 'Sync failed. See details below.', ok: false });
      }
    } catch {
      setApiSyncMsg({ text: 'Network error. Check your internet connection.', ok: false });
    }
    setApiSyncing(false);
  };
    ...
    <div className="space-y-5 max-w-7xl">
      ...
      {platformResults.length > 0 && (
        <div className="rounded-2xl overflow-hidden" style={{ border: '1px solid var(--border)' }}>
          <div className="px-4 py-2.5 text-[10px] font-bold uppercase tracking-wider" style={{ background: 'var(--surface)', borderBottom: '1px solid var(--border)', color: 'var(--muted)' }}>
            Sync Results Per Platform
          </div>
          {platformResults.map((pr: any) => (
            <div key={pr.platform} className="px-4 py-3 flex items-start gap-3" style={{ borderBottom: '1px solid var(--border)', background: 'var(--card)' }}>
              <span className="text-[10px] font-black px-2 py-0.5 rounded-full mt-0.5 shrink-0"
                style={{
                  background: pr.status === 'success' ? 'var(--success-bg)' : pr.status === 'skipped' ? 'var(--surface)' : 'var(--danger-bg)',
                  color: pr.status === 'success' ? 'var(--success)' : pr.status === 'skipped' ? 'var(--muted)' : 'var(--danger)',
                }}>
                {pr.status === 'success' ? '✓' : pr.status === 'skipped' ? '—' : '✕'}
              </span>
              <div className="flex-1 min-w-0">
                <p className="text-xs font-bold mb-0.5" style={{ color: 'var(--text)' }}>{pr.platform}</p>
                <p className="text-[10px] leading-relaxed" style={{ color: pr.status === 'error' ? 'var(--danger)' : 'var(--muted)' }}>{pr.message}</p>
              </div>
            </div>
          ))}
        </div>
