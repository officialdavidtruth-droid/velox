...
export default function AccountConnector({ workspaceId }: AccountConnectorProps) {
  ...
  const handleConnect = async (platform: PlatformType | string) => {
    ...
    try {
      ...
      if (error) {
        // Show helpful setup instructions when env var is missing
        let msg = error;
        if (error.includes('not set')) {
          const envMap: Record<string,string> = {
            TikTok:   'VITE_TIKTOK_APP_ID',
            LinkedIn: 'VITE_LINKEDIN_CLIENT_ID',
            Google:   'VITE_GOOGLE_CLIENT_ID',
            Twitter:  'VITE_TWITTER_CLIENT_ID',
            Meta:     'VITE_META_APP_ID',
          };
          const envVar = Object.entries(envMap).find(([k]) => platform.toLowerCase().includes(k.toLowerCase()))?.[1];
          if (envVar) msg = `${error} — Add ${envVar} to Vercel Environment Variables and redeploy.`;
        }
        setActionError(msg);
        setConnectingPlatform(null);
        return;
      ...
      // ── Detect popup close → reset spinner + reload accounts ────────────
      const checkClosed = setInterval(() => {
        if (popup.closed) {
          clearInterval(checkClosed);
          // Give postMessage 1s to arrive first; then reload regardless
          setTimeout(async () => {
            setConnectingPlatform(prev => prev === platform ? null : prev);
            await loadAccounts(); // Always reload — picks up accounts saved by OAuth
          }, 1200);
        }
      }, 500);
    } catch (err: any) {
  ...
  const PLATFORMS = [
    ...
    },
    {
      ...
      desc: 'Required for Website Analytics (GA4) and YouTube channel stats. If your channel is a Brand Account, choose it on Google\u2019s account picker screen before granting access.',
    },
  ];
              ...
              {loading && accounts.length === 0 ? (
                ...
                <div className="space-y-3">
                  {accounts.map(acc => {
                    ...
                    const autoRefreshes = (acc.platform === 'google' || acc.platform === 'youtube') && !!acc.refresh_token;
                    ...
                    if (!autoRefreshes && msLeft !== null) {
                      if (msLeft <= 0) timeLeftLabel = 'Expired';
                      else if (msLeft < 3600000) timeLeftLabel = `${Math.max(1, Math.round(msLeft / 60000))}m left`;
                      else if (msLeft < 86400000) timeLeftLabel = `${Math.round(msLeft / 3600000)}h left`;
                      else timeLeftLabel = `${Math.round(msLeft / 86400000)}d left`;
                    }
                    return (
                              ...
                              {timeLeftLabel !== null && (
                                <>
                                  ...
                                  <span className="text-[10px] flex items-center gap-1"
                                    style={{ color: msLeft !== null && msLeft < 86400000 ? 'var(--warning)' : 'var(--muted)' }}>
          ...
          <div className="rounded-xl p-4 flex items-start gap-3"
            ...
            <div>
              ...
              <p className="text-[10px] leading-relaxed" style={{ color: 'var(--text-soft)' }}>
                Velox Space uses OAuth 2.0 and never stores your social media passwords. Tokens are encrypted at rest.
