...
app.get('/api/social-accounts/oauth/url', async (req, res) => {
  ...
  if ((p === 'meta' || p === 'facebook' || p === 'instagram' || p === 'meta_ads')) {
    ...
    if (appId) {
      return res.json({ url: `https://www.facebook.com/v18.0/dialog/oauth?client_id=${appId}&redirect_uri=${encodeURIComponent(redir)}&scope=pages_show_list,pages_read_engagement,business_management,ads_read&state=${state}&response_type=code&auth_type=reauthenticate` });
    }
